...
class RData:
    ...
    def split_data(self):
        ...
        for reframed in self.data['reframed']:
            ...
            nb_samples, nb_series = train.shape
            print('\n\nTimeseries ({} samples by {} series):\n'.format(nb_samples, nb_series), train)
            ...
            print(te_X.shape, len(te_X), te_y.shape)
...
class RModel:
    ...
    def fit_lstm(self, nb_epoch=1, model_name="model-01-weights.best.hdf5"):
        ...
        for i in range(nb_epoch):
            ...
            self.model.fit(
                train_X,
                train_y,  # label for the targeted state
                validation_data=(
                    test_X,
                    test_y),
                epochs=1,
    ...
    def create_model(self):
        ...
        for i in range(self.n_inputs):
            ...
            lstm_input = keras.layers.Input(
                ...
                batch_shape=(26, 26, self.features),
                name=inputName)
            ...
            lstm_layer = LSTM(self.n_neurons,
                                ...
                                stateful=False,
                                return_sequences=True)(self.lstmInputs[i])
            self.lstmLayers.append(lstm_layer)

        # combined the output
        lstm_output = keras.layers.concatenate(self.lstmLayers)
        conv_layer = Convolution1D(nb_filter=nb_filter, 
                                   filter_length=filter_length, 
                                   activation='relu', 
                                   input_shape=(window_size, nb_input_series))(lstm_output)
        max_pool = MaxPooling1D()(conv_layer)
        conv_layer = Convolution1D(nb_filter=nb_filter, filter_length=filter_length, activation='relu')(max_pool)
        max_pool = MaxPooling1D()(conv_layer)
        flatten = Flatten()(max_pool)
        
            
        output = Dense(1, activation='relu',
                       name='model_output')(flatten)
        outputs = list()
        for i in range(nb_outputs):
            outputs.append(output)        

        model = keras.models.Model(inputs=self.lstmInputs, outputs=outputs)
        start = time.time()
...
def evaluate(model, scaler, test_X, test_y):
    # make a prediction
    yhat = model.predict(test_X, batch_size=26)
    rmse = list()
    for X in test_X : 
        X = X.reshape((X.shape[0], 26)) #4*51 * self.features
...
def experiment(repeats, epochs, param):
    ...
    for r in range(repeats):
        ...
        rmse = evaluate(lstm_model, scaler, test_X, output)
